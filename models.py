...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if any(x in cfg for x in ['yolov4-tiny', 'yolov4-medium']):  # stride order reversed
                stride = [32, 16, 8]
...
class Darknet(nn.Module):
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        if self.training:  # train
            if lane_detect != None:                   
                return yolo_out, lane_detect
            else:
                return yolo_out
        elif ONNX_EXPORT:  # export
            x = [torch.cat(x, 0) for x in zip(*yolo_out)]
            if lane_detect != None:
                return x[0], torch.cat(x[1:3], 1), lane_detect  # scores, boxes: 3780x80, 3780x4, lane_detect: 200+1, 18, 4
            else:
...
def inference(output, label, opt):
    if opt.lane_use_aux:
        cls_label, seg_label = label
        cls_label, seg_label = cls_label.cuda(), seg_label.cuda()
        cls_out, seg_out = output
        return {'cls_out': cls_out, 'cls_label': cls_label, 'seg_out':seg_out, 'seg_label': seg_label}
    else:
        cls_label = label.cuda()
        cls_out = output
        return {'cls_out': cls_out, 'cls_label': cls_label}   
    
    
def resolve_val_data(results, opt):
